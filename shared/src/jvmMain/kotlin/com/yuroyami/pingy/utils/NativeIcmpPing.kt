...
/**
 * JVM (desktop) counterpart to the Android [NativeIcmpPing] — same JNI symbol
 * (`Java_com_yuroyami_pingy_utils_NativeIcmpPing_nativePingOnce`), same C
 * source (`shared/native/icmp_ping.c`), just compiled per-platform at build
 * time by the `buildJvmNative` Gradle task and packaged as resources.
 *
 ...
 * SOCK_DGRAM+IPPROTO_ICMP, so [pingOnce] returns null there.
 */
